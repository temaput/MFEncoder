...
public class MFEncoder: Encoder {
  ...
  open var dateEncodingStrategy: MFFormData.DateEncodingStrategy = .deferredToDate
  ...
  func wrapObject(_ obj: Dictionary<String, Encodable>, for additionalKey: CodingKey?) throws -> MFValue {
    ...
    for (key, value) in obj {
      result[key] = try encodeAsMFValue(value, for: additionalKey)
    }
    return .object(result)
...
struct MFKeyedEncodingContainer<K> : KeyedEncodingContainerProtocol where K : CodingKey {
  ...
  mutating func encode<T>(_ value: T, forKey key: K) throws where T : Encodable {
    let mfValue = try self.impl.encodeAsMFValue(value, for: key)
...
struct MFUnkeyedEncodingContainer: UnkeyedEncodingContainer {
  ...
  mutating func encode<T>(_ value: T) throws where T : Encodable {
    let mfValue = try impl.encodeAsMFValue(value, for: _MFCodingKey(stringValue: "Index: \(self.count)"))
    list.append(mfValue)
